...
| `/[locale]/city/[city]` | SSG, 4 paths | One city, its spots off-radar first |
...
`src/lib/scoring.ts` — `sortSpots(spots, mode)` is the single entry point for ordering; `sortByOffRadar` is the default everywhere. `offRadarBand()` maps a score to the label the meter shows. Phase 6 replaces the off-radar branch and nothing else (D4).

## Client state

`src/store/filters.ts` (Zustand): `city`, `categories[]`, `sort`, `selectedId`, `hoveredId`. Consumed only by `DiscoverView`; the map and cards take props. Initial `sort` is `"off-radar"` and must stay that way.
...
No API routes, no database, no auth, no realtime channels. Phase 3 adds the first of these and this document grows then — the Postgres schema, the auth boundary and the Realtime channel names for Phase 4's voting all belong here.
